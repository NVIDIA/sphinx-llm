...
| **Build-time behavior**                 | **New** Now has minimal build overhead. A separate build of the markdown is run in parallel, then the two build outputs are merged. | Minimal build time impact; post build runs a converter/aggregator of `_sources`.               |
